            ...
            {isOpen && (
                <div className="absolute left-0 mt-2 w-full origin-top-right bg-white rounded-md shadow-lg min-w-48 z-10">
                    <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                        {options.map((option, index) => (<div key={index} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer" role="menuitem" onClick={() => handleSelectDropDown(option)}>{option}</div>))}
